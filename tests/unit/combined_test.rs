...
use clickhouse_filters::filtering::{FilterCondition, FilterExpression, FilterOperator, JsonFilter};
...
fn test_complete_filtering() {
    ...
    // Generate query SQL
    let query_sql = filters.query_sql("my_db", "users", &["name", "age"]).unwrap();
...
fn test_combined_filters() {
    ...
    let tags_condition = FilterCondition::array_has(
        "tags", 
        "premium"
    );
