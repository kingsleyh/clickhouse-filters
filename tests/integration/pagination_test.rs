...
async fn test_basic_pagination() -> Result<()> {
    run_with_clickhouse(|client| async move {
        ...
        // Create filters
        let filters = ClickHouseFilters::new(Some(pagination), vec![], None, columns)?;
...
async fn test_second_page_pagination() -> Result<()> {
    run_with_clickhouse(|client| async move {
        ...
        let first_page_result = client.query(&first_page_sql).fetch_all::<String>().await?;
...
async fn test_last_page_pagination() -> Result<()> {
    run_with_clickhouse(|client| async move {
        ...
        let count: u64 = client
            .query("SELECT COUNT(*) FROM test_filters.users")
            .fetch_one::<u64>()
            .await?;
...
async fn test_pagination_with_counting() -> Result<()> {
    run_with_clickhouse(|client| async move {
        ...
        // Create filters
        let mut filters = ClickHouseFilters::new(Some(pagination), vec![], None, columns)?;
