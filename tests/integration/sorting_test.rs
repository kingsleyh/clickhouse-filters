...
async fn run_query(client: &Client, sorting: &Sorting) -> Result<Vec<String>> {
    ...
    let query = format!(
        "SELECT name FROM test_filters.users{} LIMIT 10",
        sorting.sql
    );
    println!("Executing query: {}", query);

    // Execute the query and collect results
    let result = client.query(&query).fetch_all::<String>().await?;
...
async fn test_sorting_multiple_columns() -> Result<()> {
    run_with_clickhouse(|client| async move {
        ...
        let query = format!(
            "SELECT name, age FROM test_filters.users{} LIMIT 10",
            sorting.sql
        );
        println!("Executing query: {}", query);
        ...
        let results = client.query(&query).fetch_all::<QueryResult>().await?;

        // Verify the first result is the oldest person
        assert_eq!(results[0].name, "Bob Brown");
